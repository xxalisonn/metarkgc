...
class PatternMatcher(nn.Module):
    ...
    def forward(self, query, support,pos_num):
        ...
        # scores: [batchsize,num_query]
        p_score = scores[:, :pos_num]
        n_score = scores[:, pos_num:]
...
class MetaR(nn.Module):
    ...
    def forward(self, task, iseval=False, curr_rel=''):
        ...
        support_negative_concat = self.concat_relation(support_negative.unsqueeze(2), pos_relation)

        query_concat = self.concat_relation(query.unsqueeze(2), pos_relation)
        query_negative_concat = self.concat_relation(negative.unsqueeze(2), pos_relation)
        qry = torch.cat((query_concat, query_negative_concat), dim=1)


        # support.squeeze(2)
        # rel = self.relation_learner(support)

        rel_pos = self.pattern_learner(support_concat).unsqueeze(2)
        rel_pos = torch.mean(rel_pos,dim=1,keepdim=True)
        rel_neg = self.pattern_learner(support_negative_concat).unsqueeze(2)
        rel_neg = torch.mean(rel_neg, dim=1, keepdim=True)
        rel_query = self.pattern_learner(qry).unsqueeze(2)

        rel_pos.retain_grad()

        # relation for support
        rel_s = rel_pos.expand(-1, few+num_sn, -1, -1)
        ...
        if iseval and curr_rel != '' and curr_rel in self.rel_q_sharing.keys():
            ...
            if not self.abla:
                ...
                grad_meta = rel_pos.grad
                rel_q = rel_pos - self.beta*grad_meta
            else:
                rel_q = rel_pos
        ...
        sc_pos_p, sc_pos_n = self.pattern_matcher(rel_query,rel_pos,num_q)
        sc_neg_p, sc_neg_n = self.pattern_matcher(rel_query,rel_neg,num_q)

        p_score = p_score + self.gamma * (sc_pos_p - sc_neg_p)
        n_score = n_score + self.gamma * (sc_pos_n - sc_neg_n)
